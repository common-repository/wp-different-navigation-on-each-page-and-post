...
define( 'WP_DIFF_NAVI_EACH_PAGE_VERSION', '1.1' );
...
if ( false !== $_REQUEST['updated'] ) : ?>
  ...
  <form method="post" action="options.php">
	...
	$blogmenus = wp_get_nav_menus( array( 'orderby' => 'name' ) );
	 ?>
<table cellpadding="0" cellspacing="0" class="wpdiffnavipostoptiontable" style="width:85%;">
   <!-- Option 1: -->
  ...
  <tr><td>
   ...
   <!-- Option 2: -->
   <tr><td><label><?php _e( 'Enter Custom css here : ' ); ?></label></td></tr>
  <tr><td>
            <textarea id="theme_settings[wpdiffnavicustomcss]" name="theme_settings[wpdiffnavicustomcss]" style="width:100%; height:350px; resize:none; padding:10px;"><?php echo $options['wpdiffnavicustomcss']; ?></textarea><br />
            <small for="theme_settings[wpdiffnavicustomcss]"><?php _e( 'Enter Custom css here' );; ?></small>
   </td>
   </tr>
</table>   
...
/*** add shortcode here ***/
function wpdnepp_shortcode() {
	global $post;

$navid = get_post_meta($post->ID, '_diffnavieachpage_nav_menu_id', true); 
if($navid){
$nav_menu =  wp_get_nav_menu_object($navid);
if($nav_menu){
$wpdnepp_output = wp_nav_menu( array( 'fallback_cb' => '', 'container_class' => 'wpdenpp_menu_outer', 'echo'=> 0, 'menu' => $nav_menu ) );
}else{
$wpdnepp_output = '<span class="wpdnepp_error_shortcode">Please select menu on current page.</span>';	
}
} else {
$wpdnepp_output = '<span class="wpdnepp_error_shortcode">Please select menu on current page.</span>';	
	}
return $wpdnepp_output;
}
add_shortcode('wpdnepp_menu', 'wpdnepp_shortcode'); 

/***** add custom css ****/

function wpdnepp_style_head() {
  $options = get_option( 'theme_settings' );
  $wpdnepp_customcss = $options['wpdiffnavicustomcss'];
  if($wpdnepp_customcss){
  echo '<style type="text/css">'. $wpdnepp_customcss .'</style>';
  }
}
add_action('wp_head', 'wpdnepp_style_head');